...
if [ -z ${LOCAL+x} ] || [ "$LOCAL" = "true" ]; then
    for i in $(ip addr show $DEVICE_NAME | grep "inet\b" | awk '{print $2}'); do
        while IFS="" read -r SERVER_IP || [ -n "$SERVER_IP" ]; do
            iptables-nft -t nat -D POSTROUTING -s $i -d $SERVER_IP -o $DEVICE_NAME -j MASQUERADE
...
if [ -z ${VPN+x} ]; then
    ...
    if [ "$VPN" = "true" ]; then
        for var in "${!VPN_INTERFACE_@}"; do
            ...
            while IFS="" read -r SERVER_IP || [ -n "$SERVER_IP" ]; do
                iptables-nft -t nat -D POSTROUTING -s $VPN_SUBNET -d $SERVER_IP -o $DEVICE_NAME -j MASQUERADE
            done < server-list.txt
            iptables-nft -D FORWARD -i $DEVICE_NAME -o ${!var} -m state --state RELATED,ESTABLISHED -j ACCEPT
            iptables-nft -D FORWARD -i ${!var} -o $DEVICE_NAME -j ACCEPT
        done
    fi
fi
...
while IFS="" read -r SERVER_IP || [ -n "$SERVER_IP" ]; do
    iptables-nft -t nat -S PREROUTING | grep $SERVER_IP | cut -d " " -f 2- | xargs -rL1 iptables-nft -t nat -D
done < server-list.txt
